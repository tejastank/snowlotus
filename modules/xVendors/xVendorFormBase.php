 ...
 * Description:  base form for xvendor
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): ______________________________________..
 ********************************************************************************/

class xVendorFormBase{
...
function checkForDuplicates($prefix){
	...
	$focus = new xVendor();
	$query = '';
	$baseQuery = 'select id, name, website, billing_address_city  from xvendors where deleted!=1 and ';
...
function buildTableForm($rows, $mod='xVendors'){
	if(!ACLController::checkAccess('xVendors', 'edit', true)){
		return '';
	}
	global $action;
	if(!empty($mod)){
	...
	{
		$form = "<form action='index.php' method='post' id='dupxVendors'  name='dupxVendors'><input type='hidden' name='selectedxVendor' value=''>";
		$form .= '<table width="100%"><tr><td>'.$mod_strings['MSG_DUPLICATE']. '</td></tr><tr><td height="20"></td></tr></table>';
		unset($_POST['selectedxVendor']);
	}
	else
	{
	...
	if ($action == 'ShowDuplicates') {
		...
		$form .= "<input type='hidden' name='selectedxVendor' id='selectedxVendor' value=''><input title='${app_strings['LBL_SAVE_BUTTON_TITLE']}' accessKey='${app_strings['LBL_SAVE_BUTTON_KEY']}' class='button' onclick=\"this.form.action.value='Save';\" type='submit' name='button' value='  ${app_strings['LBL_SAVE_BUTTON_LABEL']}  '>\n";
		...
		$form .= "<input type='submit' class='button' name='ContinuexVendor' value='${mod_strings['LNK_NEW_XVENDOR']}'>\n";
	}
	...
	foreach($rows as $row){
		...
		if ($action != 'ShowDuplicates')
		{
		$form .= "<td width='1%' nowrap><a href='javascript:void(0)' onclick='document.dupxVendors.selectedxVendor.value=\"${row['id']}\"; document.dupxVendors.submit(); '>[${app_strings['LBL_SELECT_BUTTON_LABEL']}]</a>&nbsp;&nbsp;</td>\n";
		}
		foreach ($row as $key=>$value){
				if($key != 'id'){
                    if(isset($_POST['popup']) && $_POST['popup']==true){
                        $form .= "<td scope='row'><a  href='javascript:void(0)' onclick=\"window.opener.location='index.php?module=xVendors&action=DetailView&record=${row['id']}'\">$value</a></td>\n";
                    }   
                    else
					    $form .= "<td><a target='_blank' href='index.php?module=xVendors&action=DetailView&record=${row['id']}'>$value</a></td>\n";
	...
	if ($action == 'ShowDuplicates') {
		$return_action = 'ListView'; // cn: bug 6658 - hardcoded return action break popup -> create -> duplicate -> cancel
		$return_action = (isset($_REQUEST['return_action']) && !empty($_REQUEST['return_action'])) ? $_REQUEST['return_action'] : $return_action;
		$form .= "<input type='hidden' name='selectedxVendor' id='selectedxVendor' value=''><input title='${app_strings['LBL_SAVE_BUTTON_TITLE']}' class='button' onclick=\"this.form.action.value='Save';\" type='submit' name='button' value='  ${app_strings['LBL_SAVE_BUTTON_LABEL']}  '>\n";
		...
		$form .= "<input type='submit' class='button' name='ContinuexVendor' value='${mod_strings['LNK_NEW_XVENDOR']}'></form>\n";
	}
...
$the_form .= <<<EOQ
		<form name="${prefix}xVendorSave" onSubmit="return check_form('${prefix}xVendorSave');" method="POST" action="index.php">
			<input type="hidden" name="${prefix}module" value="xVendors">
			<input type="hidden" name="${prefix}action" value="Save">
EOQ;
$the_form .= $this->getFormBody($prefix, $mod, $prefix."xVendorSave");
$the_form .= <<<EOQ
...
$lbl_xvendor_name = $mod_strings['LBL_XVENDOR_NAME'];
...
EOQ;
	$form .= "$lbl_xvendor_name&nbsp;<span class='required'>$lbl_required_symbol</span><br><input name='name' type='text' value=''><br>";
...
$javascript->setSugarBean(new xVendor());
...
if (isset($contact->assigned_user_id)) {
	...
	$form .= <<<EOQ
		...
		<tr>
		<td width="20%" nowrap scope="row">$lbl_xvendor_name&nbsp;<span class="required">$lbl_required_symbol</span></td>
		<TD width="80%" nowrap scope="row">{$mod_strings['LBL_DESCRIPTION']}</TD>
		</tr>
		<tr>
		<td nowrap ><input name='{$prefix}name' type="text" value="$contact->xvendor_name"></td>
...
EOQ;
	//carry forward custom lead fields common to xvendors during Lead Conversion
	$tempxVendor = new xVendor();
	if (method_exists($contact, 'convertCustomFieldsForm')) $contact->convertCustomFieldsForm($form, $tempxVendor, $prefix);
	unset($tempxVendor);
$form .= <<<EOQ
		</TABLE>
EOQ;
