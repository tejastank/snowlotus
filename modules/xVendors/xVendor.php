 ...
 * Description:  Defines the xVendor SugarBean xVendor entity with the necessary
 * methods and variables.
 ********************************************************************************/

require_once("include/SugarObjects/templates/company/Company.php");

// xVendor is used to store xvendor information.
class xVendor extends Company {
	...
	var $xvendor_name = '';
	var $bug_id ='';
	var $module_dir = 'xVendors';
	var $emailAddress;


	var $table_name = "xvendors";
	var $object_name = "xVendor";
	var $importable = true;
	var $new_schema = true;
	...
	function xVendor() {
        ...
        $this->setupCustomFields('xVendors');
	...
	function clear_xvendor_case_relationship($xvendor_id='', $case_id='')
	{
		if (empty($case_id)) $where = '';
		else $where = " and id = '$case_id'";
		$query = "UPDATE cases SET xvendor_name = '', xvendor_id = '' WHERE xvendor_id = '$xvendor_id' AND deleted = 0 " . $where;
		$this->db->query($query,true,"Error clearing xvendor to case relationship: ");
	}
	...
	* @deprecated.
	...
	function fill_in_additional_detail_fields()
	{
        ...
        if(empty($this->parent_name) && !empty($this->id)){
			$query = "SELECT a1.name from xvendors a1, xvendors a2 where a1.id = a2.parent_id and a2.id = '$this->id' and a1.deleted=0";
...
//		$temp_array["ENCODED_NAME"]=htmlspecialchars($this->name, ENT_QUOTES);
	...
	*/
	...
	array_push($where_clauses, "xvendors.name like '$the_query_string%'");
	if (is_numeric($the_query_string)) {
		array_push($where_clauses, "xvendors.phone_alternate like '%$the_query_string%'");
		array_push($where_clauses, "xvendors.phone_fax like '%$the_query_string%'");
		array_push($where_clauses, "xvendors.phone_office like '%$the_query_string%'");
	}
        ...
        function create_export_query(&$order_by, &$where, $relate_link_join='')
        {
			...
			if($custom_join)
				$custom_join['join'] .= $relate_link_join;
                         $query = "SELECT
                                xvendors.*,email_addresses.email_address email_address,
                                xvendors.name as xvendor_name,
						...
						if($custom_join){
                         ...
                         $query .= "LEFT JOIN users
	                                ON xvendors.assigned_user_id=users.id ";

						//join email address table too.
						$query .=  ' LEFT JOIN  email_addr_bean_rel on xvendors.id = email_addr_bean_rel.bean_id and email_addr_bean_rel.bean_module=\'xVendors\' and email_addr_bean_rel.deleted=0 and email_addr_bean_rel.primary_address=1 ';
		        ...
		        $where_auto = "( xvendors.deleted IS NULL OR xvendors.deleted=0 )";
	...
	function set_notification_body($xtpl, $xvendor)
	{
		$xtpl->assign("ACCOUNT_NAME", $xvendor->name);
        $xtpl->assign("ACCOUNT_TYPE", $xvendor->xvendor_type);
		$xtpl->assign("ACCOUNT_DESCRIPTION", $xvendor->description);
